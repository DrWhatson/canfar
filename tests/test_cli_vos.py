"""Integration tests for CANFAR VOS CLI commands.

These tests require:
1. Valid CANFAR authentication (run `canfar auth login` first)
2. Access to a VOSpace server

Run with: pytest -m integration tests/test_cli_vos.py -v
...
@pytest.mark.vos
@pytest.mark.order(1)
def test_vos_mkdir(vos_test_dir: str) -> None:
    """Test creating a directory in VOSpace."""
    _test_state["test_dir"] = vos_test_dir

    result = runner.invoke(cli, ["vos", "mkdir", vos_test_dir])
...
@pytest.mark.vos
@pytest.mark.order(2)
def test_vos_ls_empty_dir() -> None:
    """Test listing an empty directory in VOSpace."""
    test_dir = _test_state.get("test_dir")
    if not test_dir:
        pytest.skip("test_vos_mkdir must run first")

    result = runner.invoke(cli, ["vos", "ls", test_dir])
...
@pytest.mark.vos
@pytest.mark.order(3)
def test_vos_cp_upload(local_test_file: str) -> None:
    ...
    result = runner.invoke(cli, ["vos", "cp", local_test_file, destination])
...
@pytest.mark.vos
@pytest.mark.order(4)
def test_vos_ls_with_file() -> None:
...
@pytest.mark.vos
@pytest.mark.order(5)
def test_vos_ls_long_format() -> None:
    """Test listing with long format (-l flag)."""
    test_dir = _test_state.get("test_dir")
    if not test_dir:
        pytest.skip("test_vos_mkdir must run first")

    result = runner.invoke(cli, ["vos", "ls", "-l", test_dir])
...
@pytest.mark.vos
@pytest.mark.order(7)
def test_vos_rm_file() -> None:
    """Test removing a file from VOSpace."""
    uploaded_file = _test_state.get("uploaded_file")
    if not uploaded_file:
        pytest.skip("test_vos_cp_upload must run first")

    result = runner.invoke(cli, ["vos", "rm", uploaded_file])
...
@pytest.mark.vos
@pytest.mark.order(8)
def test_vos_ls_after_delete() -> None:
...
@pytest.mark.vos
@pytest.mark.order(9)
def test_vos_rm_directory() -> None:
    ...
    result = runner.invoke(cli, ["vos", "rm", "-R", test_dir])
...
@pytest.mark.integration
@pytest.mark.vos
def test_vos_ls_invalid_uri() -> None:
    """Test ls with an invalid VOSpace URI."""
    result = runner.invoke(cli, ["vos", "ls", "invalid://not-a-vos-uri"])
...
@pytest.mark.integration
@pytest.mark.vos
def test_vos_ls_nonexistent_path(vos_base_uri: str) -> None:
    """Test ls on a path that doesn't exist."""
    nonexistent = f"{vos_base_uri.rstrip('/')}/nonexistent_path_12345/"

    result = runner.invoke(cli, ["vos", "ls", nonexistent])
...
@pytest.mark.integration
@pytest.mark.vos
def test_vos_help() -> None:
    """Test vos --help command."""
    result = runner.invoke(cli, ["vos", "--help"])

    assert result.exit_code == 0
    assert "Manage files in VOSpace" in result.output
...
@pytest.mark.integration
@pytest.mark.vos
def test_vos_ls_help() -> None:
    """Test vos ls --help command."""
    result = runner.invoke(cli, ["vos", "ls", "--help"])

    assert result.exit_code == 0
    assert "List VOSpace directory contents" in result.output


@pytest.mark.integration
@pytest.mark.vos
def test_vos_cp_help() -> None:
    """Test vos cp --help command."""
    result = runner.invoke(cli, ["vos", "cp", "--help"])

    assert result.exit_code == 0
    assert "Copy files to and from VOSpace" in result.output


@pytest.mark.integration
@pytest.mark.vos
def test_vos_rm_help() -> None:
    """Test vos rm --help command."""
    result = runner.invoke(cli, ["vos", "rm", "--help"])

    assert result.exit_code == 0
    assert "Remove VOSpace files or directories" in result.output


@pytest.mark.integration
@pytest.mark.vos
def test_vos_mkdir_help() -> None:
    """Test vos mkdir --help command."""
    result = runner.invoke(cli, ["vos", "mkdir", "--help"])

    assert result.exit_code == 0
    assert "Create a new VOSpace directory" in result.output
